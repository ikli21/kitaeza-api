...
var refreshToken;
var categoryExample= {
    title:'New category', description:'Вкусная категория'
};
var categoryUpdated= {
    title:'Updated category', description:'Вкусная категория, еще и обновлена'
};
...
test('Unauthorized request', function (t) {
    request
        ...
        .end(function (err, resp) {
            t.equal(resp.status, 401, 'response status shoud be 401');
            t.end();
        });
});
...
test('Create article', function (t) {
    request
        ...
        .set('Authorization', 'Bearer ' + accessToken)
        .end(function (err, res) {
            t.equal(res.status, 200, 'response artic status shoud be 200');
...
var categoryId;
test('Create category', function (t) {
    request
        .post(baseUrl + '/categories')
        .send(categoryExample)
        .set('Authorization', 'Bearer ' + accessToken)
        .end(function (err, res) {
            t.equal(res.status, 200, 'response status shoud be 200');
            if ('article' in res.body) {
                t.equal(res.body['category']['title'], categoryExample['title'], 'created article title shoud be correct');
...
test('Check created category', function (t) {
    request
        ...
        .end(function (err, res) {
            t.equal(res.status, 200, 'response status shoud be 200');
            if ('article' in res.body) {
                t.equal(res.body['category']['title'], categoryExample['title'], 'created category title shoud be correct');
                t.equal(res.body['category']['description'], categoryExample['description'], 'created description shoud be correct');
            }
            t.end();
        });
});
...
test('Update category', function (t) {
    request
        ...
        .end(function (err, res) {
            t.equal(res.status, 200, 'response status shoud be 200');
            if ('category' in res.body) {
                t.equal(res.body['category']['title'], categoryUpdated['title'], 'updated category title shoud be correct');
            }
            t.end();
        });
});

test('Test categories list', function (t) {
    request
        ...
        .end(function (err, res) {
            ...
            for (var i = 0; i < res.body.length; i++) {
                var category = res.body[i];
                if (category['_id'] === categoryId) {
                    categoryFound = true;
                    t.equal(category['title'], categoryUpdated['title'], 'updated category title shoud be correct');
                }
            }
            t.true(categoryUpdated, 'created/updated category shoud be in a list');
            t.end();
        });
});
